...
internal class GdxClientRuntime(
    ...
    fun cancelArmedMode(): Boolean {
        val hadArmedMode = groundMode != null || buildModeTypeId != null
        groundMode = null
        buildModeTypeId = null
        return hadArmedMode
    }
    ...
    fun actionHint(actionId: String): String? =
        when (actionId) {
            "pause" -> "Toggle sim pause [Space]"
            "slower" -> "Sim speed down [[]"
            "faster" -> "Sim speed up []]"
            "debug" -> "Toggle extra info [Tab]"
            "clear" -> "Clear selection [Esc]"
            "centerSelection" -> "Center on selection [Home]"
            "centerFaction" -> "Center on current view [End]"
            "viewF1" -> "View faction 1 [1]"
            "viewF2" -> "View faction 2 [2]"
            "observer" -> "View observer [3]"
            "selectViewedFaction" -> "Select current faction [F2]"
            "selectType" -> "Select same type [F3]"
            "selectRole" -> "Select same role [F4]"
            "selectAll" -> "Select all units [F11]"
            "selectIdleWorkers" -> "Select idle workers [F12]"
            "selectDamaged" -> "Select damaged units [F]"
            "selectCombat" -> "Select combat units [V]"
            "selectProducers" -> "Select producer buildings [N]"
            "selectTrainers" -> "Select training buildings [Z]"
            "selectResearchers" -> "Select research buildings [C]"
            "selectConstruction" -> "Select active construction [J]"
            "selectHarvesters" -> "Select active harvesters [K]"
            "selectReturning" -> "Select returning harvesters [Q]"
            "selectCargo" -> "Select loaded harvesters [E]"
            "selectDropoffs" -> "Select resource dropoffs [D]"
            "saveQuick" -> "Save quick preset"
            "loadQuick" -> "Load quick preset"
            "saveAlt" -> "Save alt preset"
            "loadAlt" -> "Load alt preset"
            "help" -> "Toggle help [F1]"
            "move" -> "Arm move [M] then LMB/RMB"
            "attackMove" -> "Arm attack [A] then LMB/RMB"
            "patrol" -> "Arm patrol [P] then LMB/RMB"
            "hold" -> "Hold position [H]"
            "cancelBuild" -> "Cancel build [X]"
            "cancelTrain" -> "Cancel train [T]"
            "cancelResearch" -> "Cancel research [Y]"
            else ->
                when {
                    actionId.startsWith("build:") -> "Place ${actionId.removePrefix("build:")} then LMB/RMB"
