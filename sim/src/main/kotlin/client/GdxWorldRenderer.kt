...
internal class GdxWorldRenderer(
            ...
            if (entity.weaponCooldownTicks > 0 && entity.weaponId != null) {
                ...
                hostile?.let { target ->
                    ...
                    if (isMelee) {
                        ...
                        shape.color = if (marineStyle) Color(1.00f, 0.92f, 0.74f, 0.24f) else Color(1.00f, 0.82f, 0.58f, 0.22f)
                        shape.circle(tailX, tailY, if (marineStyle) 4.8f else 6.2f)
                        shape.color = if (marineStyle) Color(0.76f, 0.78f, 0.82f, 0.12f) else Color(0.40f, 0.38f, 0.34f, 0.14f)
                        shape.circle(targetX + 2f, targetY + 3f, if (marineStyle) 3.4f else 4.2f)
                        shape.circle(targetX - 3f, targetY + 5f, if (marineStyle) 2.6f else 3.4f)
    ...
    private fun drawGroundPing(shape: ShapeRenderer, runtime: GdxClientRuntime) {
        ...
        when (ping.kind) {
            GroundPingKind.MOVE -> {
                val outer = 12f + (pulse * 10f)
                val inner = 5f + (pulse * 5f)
                ...
                shape.circle(x, y, 16f + (pulse * 6f))
                shape.rect(x - 0.75f, y - 6f, 1.5f, 12f)
                shape.rect(x - 6f, y - 0.75f, 12f, 1.5f)
            }
            GroundPingKind.ATTACK -> {
                ...
                shape.rectLine(x - 9f, y - 9f, x + 9f, y + 9f, 1.8f)
                shape.rectLine(x - 9f, y + 9f, x + 9f, y - 9f, 1.8f)
                shape.rect(x - 0.75f, y - 11f, 1.5f, 22f)
                shape.rect(x - 11f, y - 0.75f, 22f, 1.5f)
                shape.color = Color(1.00f, 0.92f, 0.72f, 0.82f)
                shape.circle(x, y, 3.8f)
            }
            GroundPingKind.BUILD -> {
                val outer = 15f + (pulse * 4f)
                shape.color = pingTone(ping.kind).cpy().mul(1f, 1f, 1f, 0.12f + (pulse * 0.08f))
                shape.rect(x - outer, y - outer, outer * 2f, outer * 2f)
                shape.color = pingTone(ping.kind).cpy().mul(1f, 1f, 1f, 0.18f + (pulse * 0.08f))
                shape.rect(x - 9f, y - 9f, 18f, 18f)
            ...
            GroundPingKind.INVALID -> {
                ...
                shape.circle(x, y, 16f + (pulse * 6f))
                shape.color = pingTone(ping.kind).cpy().mul(1f, 1f, 1f, 0.16f + (pulse * 0.08f))
                shape.circle(x, y, 9f + (pulse * 3f))
                shape.color = Color(1.00f, 0.76f, 0.76f, 0.96f)
                shape.rectLine(x - 8f, y - 8f, x + 8f, y + 8f, 1.8f)
                shape.rectLine(x - 8f, y + 8f, x + 8f, y - 8f, 1.8f)
                shape.rect(x - 9f, y - 0.75f, 18f, 1.5f)
                shape.rect(x - 0.75f, y - 9f, 1.5f, 18f)
                shape.circle(x, y, 3f)
            }
        }
    }
