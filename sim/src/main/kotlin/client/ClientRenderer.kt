...
internal fun commandButtonTooltip(actionId: String): String? =
    when {
        ...
        actionId == "select:dropoffs" -> "Select buildings that accept resource drop-offs"
...
internal fun buildCommandButtons(
    ...
    val buttons =
        mutableListOf(
            ...
            ClientCommandButton("Dropoffs", "select:dropoffs"),
...
internal fun buildSelectionArchetypeSummary(
    ...
    if (selectedIds.isEmpty()) return "selection roles: none"
    val counts = LinkedHashMap<String, Int>()
    for (entity in snapshot.entities) {
        if (entity.id !in selectedIds) continue
        val archetype = entity.archetype ?: continue
