...
class ConsoleClientTest {
    ...
    fun `console renderer summarizes current client state`() {
        ...
        assertEquals(
            "tick=9 view=f1 selected=3 entities=3 resources=0 visible[f1=6 f2=4] economy: f1 minerals=275 gas=40 dropoffs=1 selection factions: f1=3 selection hud: Marinex1 Workerx1 Depotx1 selection roles: infantryx1 workerx1 producerx1 selection classes: workers=1 combat=0 structures=0 other=2 selection pos: center=3.0,3.3 span=2.0x1.0 selection density: units=3 perTile=1.50 area=2.0 selection visibility: unknown selection hp: 185/665 (27%) selection durability: avgArmor=0.3 damaged=1/3 selection vision: none selection cargo: none selection mobility: moving=0 pathing=0 stationary=3 selection weapons: none selection paths: none orders: queued=0 active=none selection idle: total=1 workers=0 selection phases: gather=0 return=0 build=1 train=1 research=1 selection targets: build=1 harvestNodes=0 return=0 selection rally: configured=1/1 top=9,9 selection structures: none selection combat: armed=0 ready=0 cooling=0 unarmed=3 nextReady=0 selection alerts: lowHp=1 idleWorkers=0 capabilities: train=1 research=1 rally=1 dropoff=1 selection queues: prod=2@1 research=2@1 selection eta: prod=9.0 research=8.0 build=6.0 commands: move=on train=on research=on viewSelect=on builders: active=1 targets=1 construction: sites=1 remaining=6 Depotx1 tasks: build=1 gather=0 return=0 paths: none fog: unavailable production: labs=1 queue=2 active=Marinex1 research: labs=1 queue=2 active=AdvancedTrainingx1 tech: AdvancedTrainingx1 activity: builds=1/x1 buildFails=2[invalidPlacement=1,insufficientResources=1] train=q2/c1/x1 trainFails=1[queueFull=1] research=q1/c0/x1 researchFails=1[invalidTech=1] @9 construction state: total=2 f1=2 f2=0 remaining=10 @9 production events: e1/p2/c0/x1 @9 research events: e1/p2/c0/x1 @9 state: victory Enemy faction eliminated last ack: ok attackMove[cli-5] @9",
            output
        )
    }
    ...
    fun `console renderer reports observer view when no faction selected`() {
        ...
        assertTrue(output.contains("tasks: none"))
        assertTrue(output.contains("paths: none"))
        assertTrue(output.contains("fog: observer"))
        assertTrue(output.contains("state: ongoing"))
        assertTrue(output.contains("commands: move=off train=off research=off viewSelect=off"))
    }

    @Test
    fun `console renderer reports victory and defeat state summaries`() {
        val victory =
            renderClientTextFrame(
                ClientSessionState(
                    snapshot =
                        ClientSnapshot(
                            ...
                            mapHeight = 16,
                            factions = listOf(FactionSnapshot(faction = 1, visibleTiles = 6), FactionSnapshot(faction = 2, visibleTiles = 4)),
                            entities = listOf(EntitySnapshot(id = 1, faction = 1, typeId = "Marine", x = 2f, y = 2f, dir = 0f, hp = 40, maxHp = 40, armor = 0)),
                            resourceNodes = emptyList()
                        ),
                    viewedFaction = 1
                )
                ...
                ClientSessionState(
                    snapshot =
                        ClientSnapshot(
                            ...
                            mapHeight = 16,
                            factions = listOf(FactionSnapshot(faction = 1, visibleTiles = 6), FactionSnapshot(faction = 2, visibleTiles = 4)),
                            entities = listOf(EntitySnapshot(id = 2, faction = 2, typeId = "Marine", x = 2f, y = 2f, dir = 0f, hp = 40, maxHp = 40, armor = 0)),
                            resourceNodes = emptyList()
                        ),
                    viewedFaction = 1
                )
        ...
        assertTrue(victory.contains("state: victory Enemy faction eliminated"))
        assertTrue(defeat.contains("state: defeat Your faction has been eliminated"))
    }
