    ...
    fun `poll reads appended ndjson records incrementally`() {
        ...
        InputTailReader.open(path).use { reader ->
            Files.writeString(
                ...
                {"tick":1,"commandType":"build","faction":1,"typeId":"Depot","tileX":4,"tileY":4,"label":"depot"}
                """.trimIndent() + "\n",
                StandardOpenOption.APPEND
            )
            reader.poll(commandsByTick, selectionsByTick)
            ...
            Files.writeString(
                ...
                {"tick":3,"commandType":"move","units":[7,8],"x":12.0,"y":13.0}
                """.trimIndent() + "\n",
                StandardOpenOption.APPEND
            )
            reader.poll(commandsByTick, selectionsByTick)
