...
- Snapshots expose faction minerals/gas, per-faction drop-off building counts, resource nodes with remaining amounts, a dedicated `dropoffEntityIds` list, plus entity archetypes, production state, construction progress, building extents, placement clearance, building capabilities/queue limits, drop-off resource compatibility, default rally offsets, current rally point, and harvester cargo/return state when present
...
- `--snapshotOut <path>` write typed snapshot NDJSON records (`recordType`, `tick`, `snapshot`) instead of stdout
  ...
  Snapshot cadence also emits `constructionState` records with unfinished building HP and remaining build ticks.
