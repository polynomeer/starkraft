...
- `--replayStatsJson` print replay stats as JSON, including per-tick selector splits and action breakdowns
  ...
  They cannot be combined with runtime/snapshot/output flags, runtime print flags, or `--noSleep`.
...
- `--replayMetaJson` is metadata-only and cannot be combined with runtime/output flags (`--ticks`, `--replayTicks`, snapshot flags, replay write flags, `--spawnScript`, `--inputTail`, `--playControlFile`, runtime print flags, or `--noSleep`)
...
The graphical and console clients also show selected fog exposure, e.g. `selection visibility: visible=4 hidden=1`.
...
The graphical and console clients also show selected idle counts, e.g. `selection idle: total=2 workers=1`.
...
The graphical and console clients also show aggregate selected HP, e.g. `selection hp: 185/465 (39%)`.
