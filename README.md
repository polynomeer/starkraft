...
- Snapshots expose faction minerals/gas, per-faction drop-off building counts, resource nodes with remaining amounts, a dedicated `dropoffEntityIds` list, plus entity archetypes, production state, building extents, placement clearance, building capabilities/queue limits, drop-off resource compatibility, default rally offsets, current rally point, and harvester cargo/return state when present
...
- `--snapshotOut <path>` write typed snapshot NDJSON records (`recordType`, `tick`, `snapshot`) instead of stdout
  ...
  They also include harvest totals for minerals/gas, pickup/deposit cycle counts and amounts, drop-off building counts by faction, plus resource-node change and depletion counts.
